...
nc_data <- nc_open('MERRA2_400.tavgM_2d_aer_Nx.201903.nc4')
# Save the print(nc) dump to a text file

{
  sink('MERRA2_400.tavgM_2d_aer_Nx.201903.txt')
  print(nc_data)
  sink()
}
...
my.array <- ncvar_get(nc_data, "SO2SMASS") # store the data in a 3-dimensional array
dim(my.array)



dlname <- ncatt_get(nc_data,"SO2SMASS","long_name") 

dunits<- ncatt_get(nc_data,"SO2SMASS","units") 

fillvalue <- ncatt_get(nc_data,"SO2SMASS","_FillValue")
  ...
  
{
  ...
  titletext <- "SO2 Surface Mass Concentration __ENSEMBLE__ (March,2019)"
